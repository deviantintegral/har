...
final class Request implements MessageInterface
{
    ...
    {
        // Clone headers
        $this->headers = array_map(static fn (Header $h) => clone $h, $this->headers);

        // Clone cookies
        if (isset($this->cookies)) {
            $this->cookies = array_map(static fn (Cookie $c) => clone $c, $this->cookies);
        }

        // Clone query string params
        if (isset($this->queryString)) {
            $this->queryString = array_map(static fn (Params $p) => clone $p, $this->queryString);
        }
